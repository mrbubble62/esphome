...
void SH8601::setup() {
  esph_log_config(TAG, "Setting up SH8601");
  this->spi_setup();
...
void SH8601::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void SH8601::reset_params_(bool ready) {
  ...
  this->write_command_(CONTRAST, &this->contrast_, 1);
}
...
void SH8601::write_init_sequence_() {
  ...
  this->write_command_(INVERT_OFF);
  this->write_command_(PIXFMT, 0x05); // Interface Pixel Format 16bit/pixel, 0x06 18bit/pixel, 0x07 24bit/pixel
  this->write_command_(DISPLAY_ON);
  this->write_command_(WRCTRLD1, 0x28);
  this->write_command_(BRIGHTNESS, 0x00); // 0x00-0xff
  this->write_command_(CONTRAST, 0x00);  // high contrast off, 0x05 On Low, 0x06 On Medium, 0x07 On High
  delay(10);
  this->reset_params_(true);
  this->setup_complete_ = true;
  esph_log_config(TAG, "SH8601 setup complete");
}

void SH8601::set_addr_window_(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  uint8_t buf[4];
  x1 += this->offset_x_;
  x2 += this->offset_x_;
  y1 += this->offset_y_;
...
void SH8601::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr, display::ColorOrder order,
